...
export default function TwoFactorAuth({ user, onUpdate }) {
  ...
  const [system2FAEnabled, setSystem2FAEnabled] = useState(true);

  useEffect(() => {
    const fetchSystem2FAStatus = async () => {
      try {
        const token = localStorage.getItem('accessToken');
        const res = await fetch('/api/admin/settings?category=2fa', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (res.ok) {
          const data = await res.json();
          setSystem2FAEnabled(data.settings?.twoFactorEnabled === true);
        }
      } catch (error) {
        console.error('Failed to fetch system 2FA status:', error);
      }
    };
    fetchSystem2FAStatus();
  }, []);
  ...
  const ToggleSwitch = () => (
    <div className="flex items-center gap-3">
      <span className={`text-sm font-medium transition-colors duration-200 ${!isEnabled ? 'text-success font-semibold' : 'text-muted'}`}>
        Off
      </span>
      <button
        ...
        <div className="mt-4 p-3 bg-yellow-500/10 rounded-lg border border-yellow-500/20">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-muted">
              ...
              <p>Two-factor authentication is currently disabled by the system administrator. 
                Even if you enable it here, it won't be required until the admin re-enables it system-wide.</p>
            </div>
          </div>
        </div>
      )}
