...
/**
 * Generate backup codes with configurable count
...
/**
 * Get configured number of backup codes from settings
 * @returns {Promise<number>} Number of backup codes to generate
 */
export async function getConfiguredBackupCodesCount() {
  try {
    const count = await getSetting('twoFactorBackupCodesCount');
    return count || 10;
  } catch (error) {
    console.error('[2FA] Failed to get backup codes count:', error);
    return 10;
  }
}

/**
 * Generate backup codes using configured count from settings
 * @returns {Promise<string[]>} Array of backup codes
 */
export async function generateConfiguredBackupCodes() {
  const count = await getConfiguredBackupCodesCount();
