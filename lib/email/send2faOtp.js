...
/**
 * Get dynamic email transporter from database with fallback to .env
 */
async function getTransporter() {
  ...
  // Fallback to .env if database values are empty
  ...
  if (pass && typeof pass === 'string' && pass.includes(':')) {
    pass = decrypt(pass);
  }
  
  // Check if SMTP is configured
  if (!host || !user || !pass) {
    console.error('[2FA EMAIL] SMTP not configured. Please check email settings.');
    return null;
  }
  
  console.log('[2FA EMAIL] Creating transporter with host:', host, 'port:', port);
...
/**
 * Get dynamic sender with site name from database
 */
async function getSender() {
  const siteName = await getSetting('siteName') || 'Nemo Auth';
  const fromEmail = await getSetting('emailFrom') || process.env.EMAIL_FROM || process.env.SMTP_USER;
...
export async function send2faOtpEmail(email, otp, firstName) {
  try {
    const siteName = await getSetting('siteName') || 'Nemo Auth';
    const siteDescription = await getSetting('siteDescription') || 'Enterprise Authentication System';
    const sender = await getSender();
    const transporter = await getTransporter();
    
    if (!transporter) {
      console.error('[2FA EMAIL] No transporter available. Skipping email to:', email);
      return false;
    }
    
    console.log('[2FA EMAIL] Sending OTP to:', email);
    console.log('[2FA EMAIL] OTP:', otp);
...
export async function sendBackupCodesEmail(email, backupCodes, firstName) {
  try {
    ...
    if (!transporter) {
      console.error('[BACKUP CODES EMAIL] No transporter available. Skipping email to:', email);
      return false;
    }
    ...
    console.log('[BACKUP CODES EMAIL] Sent successfully to:', email);
    return true;
  } catch (error) {
