...
export async function sendRoleApplicationSubmittedEmail(email, firstName, roleName, applicationId) {
  ...
  if (!enabled) {
    console.log('[Notification] Email notifications disabled, skipping role application email to:', email);
    return true;
  }
...
export async function sendRoleApprovalEmail(email, firstName, roleName, approvedBy, approvedAt) {
  const enabled = await areEmailNotificationsEnabled();
  if (!enabled) {
    console.log('[Notification] Email notifications disabled, skipping role approval email to:', email);
    return true;
  }
  ...
  try {
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Role Application Approved - ${siteName}</title>
        <style>
          body {
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
            line-height: 1.6;
...
export async function sendRoleRejectionEmail(email, firstName, roleName, rejectedBy, rejectedAt, reason) {
  const enabled = await areEmailNotificationsEnabled();
  if (!enabled) {
    console.log('[Notification] Email notifications disabled, skipping role rejection email to:', email);
    return true;
  }
  ...
  try {
    ...
    const htmlContent = `
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Role Application Update - ${siteName}</title>
        <style>
          body {
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
            line-height: 1.6;
