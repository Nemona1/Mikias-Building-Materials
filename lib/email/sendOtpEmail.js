// OTP Email Service for Password Changes, Security Alerts, and Password Changed Confirmations
...
export async function sendPasswordChangeOtp(email, otp, firstName) {
  ...
  if (!enabled) {
    ...
    console.log(`[OTP] Your password change OTP would be: ${otp} (email disabled)`);
    return true;
  }
  ...
  try {
    ...
    if (!transporter) {
      console.error('[OTP EMAIL] No transporter available. Skipping email to:', email);
      return false;
    }
    ...
    const textContent = `
      Password Change Verification - ${siteName}
...
export async function sendSecurityAlertEmail(email, firstName, alertType, details) {
  const enabled = await areEmailNotificationsEnabled();
  if (!enabled) {
    console.log('[Notification] Email notifications disabled, skipping security alert email to:', email);
    return true;
  }
  ...
  try {
    ...
    if (!transporter) {
      console.error('[SECURITY ALERT] No transporter available. Skipping email to:', email);
      return false;
    }
...
export async function sendPasswordChangedEmail(email, firstName) {
  const enabled = await areEmailNotificationsEnabled();
  if (!enabled) {
    console.log('[Notification] Email notifications disabled, skipping password changed confirmation email to:', email);
    return true;
  }
  ...
  try {
    ...
    if (!transporter) {
      console.error('[PASSWORD CHANGED] No transporter available. Skipping email to:', email);
      return false;
    }
